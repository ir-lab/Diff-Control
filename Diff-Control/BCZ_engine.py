...
class Engine:
    def __init__(self, args, logger):
        ...
        if self.args.mode.model_zoo == "BCZ":
            self.model = ModAttn_ImageBC()
    ...
    def train(self):
        ...
        if self.args.train.dataset == "Tomato" or self.args.train.dataset == "UR5_real":
        ...
        while epoch < self.args.train.num_epochs:
            ...
            for data in dataloader:
                ...
                pred = self.model(images, text_features)
            ...
            if self.global_step != 0 and (epoch + 1) % self.args.train.save_freq == 0:
                ...
                torch.save(
                    ...
                    os.path.join(
                        ...
                        "v1.0-BCZ-{}".format(self.global_step),
                    ),
                )
    ...
    def online_test(self):
        ...
        for data in test_dataloader:
            data = [item.to(self.device) for item in data]
            (prior_action, action, obs, img) = data
            text_features = obs
            images = img
            if step == 13:
                while count < 10:
        ...
        save_data["prior"] = prior_save

        save_path = os.path.join(
            self.args.train.eval_summary_directory,
            self.args.train.model_name,
            "v1.0-test-{}.pkl".format(2),
        )
